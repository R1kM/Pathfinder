...
public class JPreCondition {
	...
	public JPathCondition createPathCondition(String assumeString,
	...
	public JPathCondition addConstraints(JPathCondition pc, String assumeString,
			Map<String,Expression<?>> expressionMap) {
	...
	private void addExpression(String expressionString,
		...
		String lhs = "";
		String rhs = "";
		Expression<Integer> lhsExpression = null;
		Expression<Integer> rhsExpression = null;
		int lhsInt, rhsInt;
		double lhsDouble, rhsDouble;

		if (expressionString.contains("!=")){
			st = new StringTokenizer(expressionString,"!=");
			c= NumericComparator.NE;
			lhs = st.nextToken();
			rhs = st.nextToken();
		}else if (expressionString.contains("==")){
			st = new StringTokenizer(expressionString,"==");
			c = NumericComparator.EQ;
			lhs = st.nextToken();
			rhs = st.nextToken();
		}else if (expressionString.contains(">=")){
			st = new StringTokenizer(expressionString,">=");
			c = NumericComparator.GE;
			lhs = st.nextToken();
			rhs = st.nextToken();
		}else if (expressionString.contains("<=")){
			st = new StringTokenizer(expressionString,"<=");
			c = NumericComparator.LE;
			lhs = st.nextToken();
			rhs = st.nextToken();
		}else if (expressionString.contains(">")){
			st = new StringTokenizer(expressionString,">");
			c = NumericComparator.GT;
			lhs = st.nextToken();
			rhs = st.nextToken();

		}else if (expressionString.contains("<")){
			st = new StringTokenizer(expressionString,"<");
			c = NumericComparator.LT;
			lhs = st.nextToken();
			rhs = st.nextToken();
		}else
		...
		if (expressionMap.containsKey(lhs))
			lhsExpression = (Expression<Integer>)expressionMap.get(lhs);
		else {
			// expect a number here
			try{
				lhsInt = Integer.parseInt(lhs);
				lhsExpression = (Expression<Integer>)Constant.create(BuiltinTypes.SINT32, lhsInt);
			}
			catch(Exception e1){
				try {
					lhsDouble = Double.parseDouble(lhs);
					lhsExpression = (Expression<Integer>)CastExpression.create(Constant.create(BuiltinTypes.DOUBLE, lhsDouble), BuiltinTypes.SINT32);
				}
				catch(Exception e2) {
		...
		if (expressionMap.containsKey(rhs))
			rhsExpression = (Expression<Integer>)expressionMap.get(rhs);
		else {
			//expect a number here
			try{
				rhsInt = Integer.parseInt(rhs);
				rhsExpression = (Expression<Integer>)Constant.create(BuiltinTypes.SINT32, rhsInt);
			}
			catch(Exception e1){
				try {
					rhsDouble = Double.parseDouble(rhs);
					rhsExpression = (Expression<Integer>)CastExpression.create(Constant.create(BuiltinTypes.DOUBLE, rhsDouble), BuiltinTypes.SINT32);
				}
				catch(Exception e2) {
		...
		pc._addDet(new NumericBooleanExpression(lhsExpression, c, rhsExpression));
	}
}
